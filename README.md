...
## 六个 CME 期货品种的部分复现与 2021-2025 年样本外检验

Intraday Momentum in CME Futures: A Partial Replication and Post-2020 Out-of-Sample Evaluation

本项目对 Baltussen、Da、Lammers 和 Martens 的论文
《Hedging Demand and Market Intraday Momentum》中期货日内动量的核心回归框架进行六品种部分复现，
并使用 2021-2025 年数据检验其样本外稳定性。项目并不尝试完整复制
原论文覆盖的全部市场、历史时期、机制分析和扩展结果。

论文主要研究的是：期货市场在一天内前面几个时间段的价格变化，
能不能解释或预测最后半小时的价格变化。

我使用 Databento 的 CME 期货一分钟数据，对六个连续期货合约分别做了单品种部分复现：
...
项目的目标不是构建一个真实交易系统，而是按照论文核心设定，
把数据处理、收益率计算、回归分析和结果整理跑通，并记录与原论文实现上的差异。

## 与原论文的区别

更完整的说明见 [`docs/paper_vs_project.md`](docs/paper_vs_project.md)。
...
docs/
  ...
  paper_vs_project.md               # 原论文和本项目的差异说明
...
这样可以让论文重叠期和后面的样本外检验分开。

## 主要结果

在论文重叠期，ES、NQ、GC 和 ZN 的 Eq. (7) 回归结果为正，且在名义显著性水平下显著。
CL 和 6E 未能呈现显著的正向关系。
...
这个项目主要是课程学习性质的论文部分复现，还有很多地方可以继续改进：
